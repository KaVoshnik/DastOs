...
protected_mode:
    ...
    mov byte [0xB8000], 'B'  ; 'B' означает, что мы в защищенном режиме
    ...
    mov esi, 0x10000
    mov edi, 0x100000
    mov ecx, 50 * 512 / 4  ; Делим на 4, так как копируем двойными словами
    rep movsd

    ; ВЫВЕДЕМ ЕЩЕ ОДИН СИМВОЛ
    mov byte [0xB8002], 'K'  ; 'K' означает, что копирование завершено
